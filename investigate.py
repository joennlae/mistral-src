...
for i in range(9):
    ax = axes[0, i]
    im = heatmap(scores[i][0][0].cpu().numpy(), [], [], ax=ax, cmap="hot_r")
    annotate_heatmap(im, valfmt="{x:.2f}", size=9)

for i in range(9):
    ax = axes[1, i]
    im = heatmap(scores_softmax[i][0][0].cpu().numpy(), [], [], ax=ax, cmap="hot_r")
    annotate_heatmap(im, valfmt="{x:.2f}", size=9)

for i in range(9):
    ax = axes[2, i]
    im = heatmap(actual_scores[i][0][0].cpu().numpy(), [], [], ax=ax, cmap="hot_r")
    annotate_heatmap(im, valfmt="{x:.2f}", size=9)
